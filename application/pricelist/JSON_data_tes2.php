...
while($data = mysql_fetch_assoc($query))
{
    ...
    if($cek_hrg = mysql_query("SELECT * FROM tb_wholesale where id_produk = ".$data['kode_produk'] ." order by `harga_wholesale` DESC ")){
        ...
        if($jml_hrga > 0 ){
            $arr_data[] = '<a href="#'.$data['kode_produk'].'" data-toggle="collapse" class="btn btn-success btn-block" onClick="buka_harga('.$data['kode_produk'].')">'."Rp. " . number_format($data['harga'],0,'',',').' <i class="fas fa-chevron-circle-down icon-down"></i> </a> 
                <div id="'.$data['kode_produk'].'"  class="collapse"> <b class="text-danger">Tunggu Mengambil data</b></div>';
        ...
        // harga luar angkasa
        if($jml_hrga > 0 ){
            $arr_data[] = '<a href="#'.$data['kode_produk'].'_luar" data-toggle="collapse" style="background:#DC5F00;border:none !important" class="btn btn-success btn-block" onClick="buka_harga_luar('.$data['kode_produk'].')">'."Rp. " . number_format($data['harga_luar_jawa'],0,'',',').' <i class="fas fa-chevron-circle-down icon-down"></i> </a> 
            <div id="'.$data['kode_produk'].'_luar"  class="collapse"> <b class="text-danger">Tunggu Mengambil data</b></div>';
        }else{
            $arr_data[] = '<a href="#'.$data['kode_produk'].'"  class="btn btn-success btn-block disabled"   style="background:#DC5F00;border:none !important">'."Rp. " . number_format($data['harga_luar_jawa'],0,'',',').'</a> <div id="'.$data['kode_produk'].'"  class="collapse"></div>';
