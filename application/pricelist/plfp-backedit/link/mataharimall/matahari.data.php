  ...
  <table class="table table-striped table-hover table-bordered" cellpadding="0" cellspacing="0">
      <thead>
        <tr class="success">
          ...
          <th style="text-align:center;">Link MatahariMall</th>
      ...
      <tbody>
        <?php
          ...
          SELECT p.judul_produk, mm.kode_produk, ktg.nama_ktg, mm.id, mm.link, mm.update_time FROM link_mataharimall mm
          INNER JOIN tb_pricelist p ON p.kode_produk = mm.kode_produk
          INNER JOIN tb_kategori ktg ON ktg.id_ktg = p.ktg_produk ORDER BY mm.update_time DESC
          "));
          $jml_halaman = ceil($jml_data / $jml_per_halaman);
          if(isset($_POST['src'])) {
              $kunci = $_POST['src'];
              if(ctype_digit($kunci)){
                $q = "SELECT p.judul_produk, mm.kode_produk, ktg.nama_ktg, mm.id, mm.link, mm.update_time FROM link_mataharimall mm
                      INNER JOIN tb_pricelist p ON p.kode_produk = mm.kode_produk
                      INNER JOIN tb_kategori ktg ON ktg.id_ktg = p.ktg_produk WHERE mm.kode_produk LIKE '%$kunci%' ORDER BY mm.update_time DESC";
                $link = mysql_query($q);
              } else {
                $q = "SELECT p.judul_produk, mm.kode_produk, ktg.nama_ktg, mm.id, mm.link, mm.update_time FROM link_mataharimall mm
                      INNER JOIN tb_pricelist p ON p.kode_produk = mm.kode_produk
                      INNER JOIN tb_kategori ktg ON ktg.id_ktg = p.ktg_produk WHERE p.judul_produk LIKE '%$kunci%' ORDER BY mm.update_time DESC";
                $link = mysql_query($q);
              }
              ...
              $link = mysql_query("SELECT p.judul_produk, mm.kode_produk, ktg.nama_ktg, mm.id, mm.link, mm.update_time FROM link_mataharimall mm
                      INNER JOIN tb_pricelist p ON p.kode_produk = mm.kode_produk
                      INNER JOIN tb_kategori ktg ON ktg.id_ktg = p.ktg_produk ORDER BY mm.update_time DESC LIMIT $offset, $jml_per_halaman");
          // query ketika tidak ada parameter halaman maupun pencarian
          } else {
            //~ var_dump("test");
            $link = mysql_query("SELECT p.judul_produk, mm.kode_produk, ktg.nama_ktg, mm.id, mm.link, mm.update_time FROM link_mataharimall mm
                      INNER JOIN tb_pricelist p ON p.kode_produk = mm.kode_produk
                      INNER JOIN tb_kategori ktg ON ktg.id_ktg = p.ktg_produk ORDER BY mm.update_time DESC LIMIT 0, $jml_per_halaman");
            $halaman = 1;
          }
...
<script type="text/javascript">
  ...
  $('.edit').on("click", function(){
      ...
      var id_mm = this.id;
      if(id_mm != 0)
          $("#myModalLabel").html("Ubah Data Link MatahariMall");
