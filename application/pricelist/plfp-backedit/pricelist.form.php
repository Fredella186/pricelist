      ...
      <div class="formImage col-sm-6">
        ...
        <?php } else { ?>
				<div id="showUpdImage" style="display: block;">
					...
					<input type="button" name="btnImgDel" id="btnImgDel" onclick="hapus_image()" class="btn btn-primary btn-sm" value="Ubah Gambar">
				</div>
  			</div>
        <?php } ?>
...
<!-- panggil file js untuk simpan data pl -->
<script src="pricelist2.js?<?=md5("YmdHis")?>"></script>
<script>
...
// 	$('#btnImgDel').on('click', function(){
    function hapus_image(){
        if(confirm('Yakin akan mengubah gambar ?')){
	...
	function doUploadProduk() {
		...
		//$('input:button[name=btnUpload]').on('click', function(){
			var file = $("[name='gambar_prod']")[0].files[0]
		    if(!file){
    			var file = $("[name='gambar_prod']")[1].files[0]
		    }
...
// 			var a = $('#gambar_prod')[0].files[0].name;
	        var a = $("[name='gambar_prod']")[0].files[0]
		    if(!a){
    			var a = $("[name='gambar_prod']")[1].files[0]
		    }
			$('#gambar_text').val(a);
